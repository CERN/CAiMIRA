...
class TestCernApp(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        cern_theme = Path(ui.app.__file__).parent.parent / 'themes' / 'cern'
        return ui.app.make_app(theme_dir=cern_theme)
...
class TestOpenApp(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        return ui.app.make_app(calculator_prefix="/mycalc")
...
class TestError500(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        class ProcessingErrorPage(ui.app.BaseRequestHandler):
            def get(self):
...
class TestCERNGenericPage(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        cern_theme = Path(ui.app.__file__).parent.parent / 'themes' / 'cern'
        app = ui.app.make_app(theme_dir=cern_theme)
        pages = [
            (r'/calculator/user-guide', ui.app.GenericExtraPage, {'active_page': 'calculator/user-guide', 'filename': 'userguide.html.j2'}),
            (r'/about', ui.app.GenericExtraPage, {'active_page': 'about', 'filename': 'about.html.j2'}),
        ]
