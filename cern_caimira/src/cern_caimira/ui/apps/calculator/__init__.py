...
import tornado.log
from caimira.calculator.profiler import CaimiraProfiler, Profilers
...
class ConcentrationModel(BaseRequestHandler):
    async def post(self) -> None:
        ...
        try:
            ...
            report_data = generate_report_results(form, model)
...
class StaticModel(BaseRequestHandler):
    async def get(self) -> None:
        ...
        report_task = executor.submit(
            report_generator.build_report, base_url, form,
            model, report_data, executor_factory=functools.partial(
                concurrent.futures.ThreadPoolExecutor,
                self.settings['report_generation_parallelism'],
            ),
        )
...
class CO2ModelResponse(BaseRequestHandler):
    async def post(self, endpoint: str) -> None:
        ...
        try:
            form = co2_validator.CO2FormData.from_dict(requested_model_config, data_registry)
        except Exception as err:
        ...
        if endpoint.rstrip('/') == 'plot':
            transition_times = co2_validator.CO2FormData.find_change_points_with_pelt(form.CO2_data)
            self.finish({'CO2_plot': co2_validator.CO2FormData.generate_ventilation_plot(form.CO2_data, transition_times),
            ...
            report_task = executor.submit(
                co2_validator.CO2FormData.build_model, form,
            )
            ...
            result['CO2_plot'] = co2_validator.CO2FormData.generate_ventilation_plot(CO2_data=form.CO2_data,
