...
import cern_caimira.apps.calculator
from cern_caimira.apps.calculator.report.virus_report import generate_permalink
...
@pytest.fixture
def app():
    return cern_caimira.apps.calculator.make_app()
...
class TestCernApp(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        ...
        return cern_caimira.apps.calculator.make_app(theme_dir=cern_theme)
...
class TestOpenApp(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        return cern_caimira.apps.calculator.make_app(calculator_prefix="/mycalc")
...
class TestError500(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        class ProcessingErrorPage(cern_caimira.apps.calculator.BaseRequestHandler):
            def get(self):
                raise ValueError('some unexpected error')
        app = cern_caimira.apps.calculator.make_app()
        page = [
