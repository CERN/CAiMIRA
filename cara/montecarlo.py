...
def present_qR_quantiles(model: MCConcentrationModel, quantile: float = 0.5) -> None:
    """
    Displays a handful of key parameters and results of qR from a given MCConcentrationModel
    :param model: The MCConcentrationModel representing the scenario to be presented
    :param quantile: the quantile of qR
    :return: Nothing, parameters are printed
    """

    qRs = (np.log10(model.infected.emission_rate_when_present()))
    log_qr = np.asarray(qRs)
    qr_values = 10 ** log_qr
    print(f"qR_{quantile} = {np.quantile(qr_values, quantile)}")
...
def compare_viruses_qr(violins: bool = True) -> None:
    ...
    positions = [1, 2, 3, 6, 8, 10, 12]
    line_positions = [5, 7, 9, 11]
    ...
    bp = ax.boxplot(data, patch_artist=True, medianprops={'linewidth': 0}, whiskerprops={'linewidth': 0},
                    positions=positions[3:], widths=[0.6]*4)
    ...
    if violins:
        ...
        perc80 = [np.log10(np.quantile(10 ** qr, 0.80)) for qr in qrs]
        ax.hlines(y=means,
                  xmin=[pos - whisker_width / 2 for pos in positions[:3]],
                  xmax=[pos + whisker_width / 2 for pos in positions[:3]],
                  colors=colors[:3])
        ax.hlines(y=perc80,
                  xmin=[pos - whisker_width / 3 for pos in positions[:3]],
                  xmax=[pos + whisker_width / 3 for pos in positions[:3]],
                  colors=colors[:3],
    ...
    ax.set_xticks([2.5, 6, 8, 10, 12])
    ax.set_xticklabels(['SARS-CoV-2', 'SARS-CoV', 'Influenza', 'Measles', 'Tuberculosis'])
    ax.hlines(y=[np.log10(970), np.log10(45)], linestyles=[(0,(1, 1))], colors=['dodgerblue', 'crimson'], xmin=0, xmax=5)

    handles = [patches.Patch(color=c, label=l) for c, l in zip([p + (0.3,) for p in pastels], ('Breathing', 'Speaking', 'Shouting'))]
    handles += [mlines.Line2D([], [], linestyle=(0, (1, 1)), color='dodgerblue', label='S V Chorale (qR=970)')]
    handles += [mlines.Line2D([], [], linestyle=(0, (1, 1)), color='crimson', label='Bus ride (qR=45)')]
    plt.legend(handles=handles, loc='lower left', bbox_to_anchor=(0.10, 0.01))
...
def plot_pi_vs_exposure_time(exp_models: typing.List[MCExposureModel], labels: typing.List[str],
    ...
    if linestyles is None:
        linestyles = ['solid' for _ in exp_models]

    presence_intervals = [m.exposed.presence.boundaries() for m in exp_models]
    all_equal = True
    first_interval = presence_intervals[0]
    for interval in presence_intervals[1:]:
        if interval != first_interval or len(interval) > 1:
            all_equal = False
            break
    ...
    for finish in tqdm(times):
        current_models = [MCExposureModel(
            concentration_model=cm,
            exposed=models.Population(
                number=em.exposed.number,
                presence=models.SpecificInterval(((start, finish), )),
                activity=em.exposed.activity,
                mask=em.exposed.mask
            )
        ) for cm, em in zip(conc_models, exp_models)]

        for i, m in enumerate(current_models):
            pis[i].append(np.mean(m.infection_probability()) / 100)
