...
import ui.apps.calculator
from ui.apps.calculator.report import generate_permalink
...
@pytest.fixture
def app():
    return ui.apps.calculator.make_app()
...
class TestCernApp(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        cern_theme = Path(ui.apps.calculator.__file__).parent.parent / 'themes' / 'cern'
        return ui.apps.calculator.make_app(theme_dir=cern_theme)
...
class TestOpenApp(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        return ui.apps.calculator.make_app(calculator_prefix="/mycalc")
...
class TestError500(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        class ProcessingErrorPage(ui.apps.calculator.BaseRequestHandler):
            def get(self):
                raise ValueError('some unexpected error')
        app = ui.apps.calculator.make_app()
        page = [
...
class TestCERNGenericPage(tornado.testing.AsyncHTTPTestCase):
    def get_app(self):
        cern_theme = Path(ui.apps.calculator.__file__).parent.parent / 'themes' / 'cern'
        app = ui.apps.calculator.make_app(theme_dir=cern_theme)
        pages = [
            (r'/calculator/user-guide', ui.apps.calculator.GenericExtraPage, {'active_page': 'calculator/user-guide', 'filename': 'userguide.html.j2'}),
            (r'/about', ui.apps.calculator.GenericExtraPage, {'active_page': 'about', 'filename': 'about.html.j2'}),
        ]
