...
@pytest.fixture
def short_range_model_list(valid_conc_model_list):
    infected = valid_conc_model_list[0].infected
    sr_activity=models.Activity.types['Seated']
    sr_expiration=models.Expiration.types['Breathing']
...
def get_exposure_model(concentration_model_list, sr_model_list = ()) -> mc.ExposureModel:
    return mc.ExposureModel(
        data_registry=data_registry,
        concentration_model=concentration_model_list,
        short_range=sr_model_list,
...
def test_sr_model_concentration(time, valid_conc_model_list, short_range_model_list):
    sr_model = short_range_model_list[0].build_model(SAMPLE_SIZE)
    concentration = sr_model.short_range_concentration(valid_conc_model_list, time)
...
@pytest.mark.parametrize("time", [0., 0.6, 1., 3., 7, 17.])
def test_sr_concentration(time, valid_conc_model_list, short_range_model_list):
    concentration = get_exposure_model(valid_conc_model_list, short_range_model_list).build_model(SAMPLE_SIZE).concentration(time)
    assert concentration >= 0
