...
class VirusFormData(FormData):
    ...
    def validate(self):
        ...
        if self.occupancy_format == "static":
            ...
            for key, group in self.dynamic_exposed_occupancy.items():
                max_occupants_group = np.max(
                    np.array([entry["total_people"] for entry in group]))
                if max_occupants_group > max_occupants_for_sr: max_occupants_for_sr = max_occupants_group
    ...
    def build_mc_model(self) -> typing.Union[mc.ExposureModel, mc.ExposureModelGroup]:
        ...
        if self.occupancy_format == 'dynamic':
            exposure_model_set = []
            for exposure_group_key, exposed_group in self.dynamic_exposed_occupancy.items():
                sr_models: typing.Tuple[models.ShortRangeModel, ...] = tuple(short_range[exposure_group_key])
                population_occupancy = self.generate_dynamic_occupancy(exposed_group)
                exposed_population: mc.Population = self.exposed_population(population_occupancy).build_model(size)
    ...
    def exposed_population(self, population_occupancy: typing.Optional[models.IntPiecewiseConstant] = None) -> mc.Population:
        """
        Generates an exposed Population class, both for static and
        dynamic occupancy. The number of people is constant for a
        single exposed population, except when breaks are defined.
        """
        ...
        if population_occupancy is not None:
            number = (set(population_occupancy.values) - {0}).pop()
            presence = population_occupancy.interval()
        else:
