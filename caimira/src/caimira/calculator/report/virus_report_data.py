...
def _concentrations_with_sr_breathing(form: VirusFormData, model: models.ExposureModel, 
    ...
    """
    for c_model in model.concentration_model_list:
        for index, (start, stop) in enumerate([interaction.presence.boundaries()[0] for interaction in c_model.short_range]):
...
def group_results(form: VirusFormData, model_group: models.ExposureModelGroup) -> typing.Dict[str, typing.Any]:
    ...
    for single_group in model_group.exposure_models:
        ...
        for concentration_model in single_group.concentration_model_list:
            if concentration_model.short_range != ():
                has_short_range = True
                for short_range_model in concentration_model.short_range:
        ...
        if has_short_range:
            long_range_single_group = dataclass_utils.nested_replace(
                single_group,
                {
                    "concentration_model": list(
                        dataclass_utils.nested_replace(cm, {"short_range": ()})
                        for cm in single_group.concentration_model_list
                    )
                },
            )
    ...
    with executor_factory() as executor:
        for time1, time2 in zip(times[:-1], times[1:]):
            for single_group in model_group.exposure_models:
                ...
                if any(c_model.short_range != () for c_model in single_group.concentration_model_list):
                    tasks.append(executor.submit(
